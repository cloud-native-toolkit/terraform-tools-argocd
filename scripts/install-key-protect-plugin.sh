...
LATEST_RELEASE=$(curl -Ls https://api.github.com/repos/cloud-native-toolkit/argocd-plugin-key-protect/releases/latest | jq -r '.tag_name')
echo "Latest release of Key Protect plugin: ${LATEST_RELEASE}"

echo "Downloading Key Protect plugin installer"
curl -Lo ${TMP_DIR}/install-plugin.tar.gz "https://github.com/cloud-native-toolkit/argocd-plugin-key-protect/releases/download/${LATEST_RELEASE}/install-plugin.tar.gz"
...
${TMP_DIR}/argocd/bin/install-plugin-dependencies.sh "${NAMESPACE}" "${NAME}"
